function srs5_image_generation(root_folder, wn_start, wn_end, image_count)
    

    arguments
        root_folder (1,1) string
        wn_start (1,1) double = 2700
        wn_end (1,1) double = 3120
        image_count (1,1) double = 62
    end

    baseDir = fileparts(mfilename('fullpath'));
    addpath(fullfile(baseDir, 'imageprocessHelper'));
    addpath(fullfile(baseDir, 'spectraprocessHelper'));
    addpath(fullfile(baseDir, 'subtype_reference'));
    
    folder_names = folders_generation(root_folder);
    
    output_folder = root_folder; mkdir(output_folder);
    
    wavenumber_samples = linspace(wn_start, wn_end, image_count); % raman shift begin, raman shift end, number of images
