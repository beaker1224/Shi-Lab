...
for i = 1: numel(subtype_name)
    ...
    % similarity
    control_nerve_similarity = max(control_nerve_dataset'*subtype_reference_spectra, [], 2);
    mutant_nerve_similarity = max(mutant_nerve_dataset'*subtype_reference_spectra, [], 2);
    p = ranksum(control_nerve_similarity, mutant_nerve_similarity);
    data = [control_nerve_similarity; mutant_nerve_similarity];
    label = [char(ones(numel(control_nerve_similarity), 1) * 'control'); char(ones(numel(mutant_nerve_similarity), 1) * 'FTLD   ')];
    ...
    exportgraphics(gca, strcat('figures/demo_6_nerve_', num2str(lb), '_', num2str(ub), '_', subtype_name(i), '_p_', num2str(p), '.emf'), 'Resolution', 2000);
    saveas(gcf,strcat('figures/demo_6_nerve_', num2str(lb), '_', num2str(ub), '_', subtype_name(i)),'fig');
    datacell = cell(numel(data), 2); datacell(:, 1) = cellstr(label); datacell(:, 2) = num2cell(data);
    writecell(datacell, strcat('figures/demo_6_nerve_', num2str(lb), '_', num2str(ub), '_', subtype_name(i), '_p_', num2str(p), '.xlsx'))
end