...
% similarity
control_neuron_similarity = max(control_neuron_dataset'*cardiolipin_reference_spectra, [], 2);
mutant_neuron_similarity = max(mutant_neuron_dataset'*cardiolipin_reference_spectra, [], 2);

data = [control_neuron_similarity; mutant_neuron_similarity];
label = [char(ones(numel(control_neuron_similarity), 1) * 'control'); char(ones(numel(mutant_neuron_similarity), 1) * 'mutant ')];
...
exportgraphics(gca, 'figures/demo_6.emf', 'Resolution', 2000);
