...
def fitted_bg_subtraction(filePath: str, inputFileType: str, outputFileType: str) -> None:
    '''
    ...
    Parameters:
    filePath (str): The path to the input CSV file containing the Raman spectrum data.'''
    Filename = filePath

    range_start = 1000
    range_end = 3900
    if inputFileType == 'csv':
        data = pd.DataFrame(pd.read_csv(Filename + '.csv', header=None)).to_numpy()
    else:
        data = pd.DataFrame(pd.read_csv(Filename + '.txt', delim_whitespace=True, header=None)).to_numpy()
    ...
    if outputFileType == 'csv':
        ...
        f = open(Filename + '_spontaneous_Raman_BG_subtracted.txt', 'w', encoding='utf-8', newline='')
